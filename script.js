class MatrixCalculator {
    ...
    calDeterminantA() {
		...
		if (this.AxDimension!=this.AyDimension) {
			...
			this.printOnConsole("Determinanta ne može biti izračunana.");
			return;
		}
		var determinant;
		...
		let det;
		if(localStorage.getItem('Determinanta(A)')===null){
			det=[];
		}else{
			det=JSON.parse(localStorage.getItem('Determinanta(A)'));
		}
		det.push(determinant);
		localStorage.setItem('Determinanta(A)',JSON.stringify(det));

		this.printOnConsole("Determinanta matrice A: "+determinant)
		return;
	}

    transposeMatrixA() {
		this.rebuildMatrix();
		var string = "Rezultat transponirane matrice A:\r";
    ...
    addMatrix() {
		this.rebuildMatrix();
		if (this.AxDimension!=this.AyDimension) {
			this.printOnConsole("Matrice imaju različite dimenzije.");
			return;
		}
		var result = [];
		for(var i=0; i<3; i++) 
		...
		var string = "Rezultat množenja:\r";
		for (i =0; i<this.AyDimension; i++) {
			for (var j=0; j<this.AxDimension; j++) {
	...
	subtractMatrix() {
		...
		var string = "Rezultat oduzimanja:\r";
		for (i =0; i<this.AyDimension; i++) {
			for (var j=0; j<this.AxDimension; j++) {
				string=string+"\t"+result[i][j];
			}
	...
	multiplyMatrix() {
		...
		if (this.AxDimension!=this.ByDimension) {
			this.printOnConsole("Broj stupaca na A razlikuje se od broja redaka na B.");
			return;
		}
		var result = [];
		for(var i=0; i<3; i++) 
	...
	calDeterminantB() {
		...
		if (this.BxDimension!=this.ByDimension) {
			...
			this.printOnConsole("Nekvadratna matrica, determinanta se ne može izračunati.");
			return;
		}
		var determinant;
		...
		let det;
		if(localStorage.getItem('Determinanta(B)')===null){
			det=[];
		}else{
			det=JSON.parse(localStorage.getItem('Determinanta(B)'));
		}
		det.push(determinant);
		localStorage.setItem('Determinanta(B)',JSON.stringify(det));
		this.printOnConsole("Determinanta matrice B: "+determinant)
		return;
	}

    transposeMatrixB() {
		this.rebuildMatrix();
		var string = "Rezultat transponirane matrice B:\r";
